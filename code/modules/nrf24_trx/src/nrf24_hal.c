...
uint8_t nrf24_read_multibyte_register(uint8_t     reg,
                                                  uint8_t*    buffer,
                                                  uint8_t     len_to_read)
{
    uint8_t status;
...
uint8_t nrf24_write_register(uint8_t reg,
                                         uint8_t value)
{
    uint8_t status;
...
*/
uint8_t nrf24_write_multibyte_register(uint8_t         reg,
...
static uint8_t nrf24_intl_get_status_value(void)
{
    return nrf24_send_command(NOP);
}

__always_inline static inline void clear_bit(uint8_t reg,
                                             uint8_t bit)
{
    nrf24_write_register(reg, nrf24_read_register(reg) &~_BV(bit));
}
...
__always_inline static inline uint8_t is_bit_set(uint8_t reg,
                                                 uint8_t bit)
{
    uint8_t value = nrf24_read_register(reg);

    return ((value & _BV(bit)) != 0);
}

#define SET_BIT(REG, BIT)   nrf24_write_register(REG, nrf24_read_register(REG) | _BV(BIT));
...
{
    CE_HIGH();
    nrf24_port_delay_us(100);
    CE_LOW();
}

uint8_t nrf24_hal_flush_rx_fifo(void)
{
    return nrf24_send_command(FLUSH_RX);
}

uint8_t nrf24_hal_flush_tx_fifo(void)
{
    return nrf24_send_command(FLUSH_TX);
}

void nrf24_hal_reuse_tx_payload(void)
{
    nrf24_write_register(STATUS_REG, _BV(MAX_RT)); //Clear max retry flag
    nrf24_send_command(REUSE_TX_PL);
}

uint8_t nrf24_hal_read_rx_payload_width(void)
{
    return nrf24_read_register(R_RX_PL_WID);
}
...
uint8_t nrf24_hal_start(void)
{
    ...
    nrf24_write_register(SETUP_ADDRESS_WIDTH_REG, 3);
...
void nrf24_hal_write_tx_payload(const uint8_t*  tx_payload,
                                uint8_t         length)
{
    nrf24_write_multibyte_register(W_TX_PAYLOAD, tx_payload, length);
}

void nrf24_hal_write_tx_payload_noack(const uint8_t*  tx_payload,
                                      uint8_t         length)
{
    nrf24_write_multibyte_register(W_TX_PAYLOAD_NOACK, tx_payload, length);
}
...
void nrf24_hal_write_ack_payload(uint8_t         pipe,
                                 ...
                                 uint8_t         length)
{
    nrf24_write_multibyte_register(W_ACK_PAYLOAD | pipe, tx_payload, length);
}
...
uint16_t nrf24_hal_read_rx_payload(uint8_t*    rx_payload)
{
    ...
    if(pipe < 7U) {
        ...
        nrf24_read_multibyte_register(R_RX_PAYLOAD, rx_payload, buffer_length);
    }
    else {
...
{
    config_t    config;
    config.value = nrf24_read_register(CONFIG_REG);
    ...
    nrf24_write_register(CONFIG_REG, config.value);
}
                         ...
                         uint8_t             auto_ack)
{
    if(pipe_no == NRF24_HAL_PIPE_ALL) {
        nrf24_write_register(ENABLE_RX_ADDR_REG, 0x3F);
        nrf24_write_register(ENABLE_AUTO_ACK_REG, (auto_ack) ? 0x3F : 0x00);
    }
    else {
...
{
    if(pipe_no == NRF24_HAL_PIPE_ALL) {
        nrf24_write_register(ENABLE_RX_ADDR_REG, 0);
        nrf24_write_register(ENABLE_AUTO_ACK_REG, 0);
    }
    else {
...
{
    return nrf24_read_register(SETUP_ADDRESS_WIDTH_REG) + 2U;
}
...
void nrf24_hal_set_auto_retransmit_delay(uint8_t ard)
{
    ...
    retry_value.value = nrf24_read_register(SETUP_RETRANSMISSION_REG);
    retry_value.bits.ard = ard;
    nrf24_write_register(SETUP_RETRANSMISSION_REG, retry_value.value);
}
...
{
    nrf24_write_register(RF_CHANNEL_REG, MIN_VALUE(channel, 125));
}

uint8_t nrf24_hal_get_channel(void)
{
    return nrf24_read_register(RF_CHANNEL_REG);
}

void nrf24_hal_set_data_rate(nrf24_hal_datarate_t    data_rate)
{
    rf_setup_t  rf_setup;
    rf_setup.value = nrf24_read_register(RF_SETUP_REG);
    ...
    nrf24_write_register(RF_SETUP_REG, rf_setup.value);
}
...
nrf24_hal_datarate_t nrf24_hal_get_data_rate ()
{
    ...
    rf_setup_reg.value = nrf24_read_register(RF_SETUP_REG);
...
void nrf24_hal_set_rf_output_power(nrf24_hal_rf_output_power_t rf_output_power)
{
    ...
    nrf24_write_register(RF_SETUP_REG, rf_setup_reg.value);
}
...
uint8_t nrf24_hal_get_clear_irq_flags(void)
{
    ...
    retval = nrf24_write_register(STATUS_REG, (_BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT)));
...
uint8_t nrf24_hal_clear_irq_flags_get_status(void)
{
    ...
    retval |= nrf24_read_register(STATUS_REG) & 0x0F;
...
{
    return (nrf24_read_register(OBSERVE_TX_REG) & 0x0F);
}
                                    ...
                                    const uint8_t*      address)
{
    if(pipe_no != NRF24_HAL_PIPE_ALL) {
        ...
        if((pipe_no < 2) || (pipe_no == NRF24_HAL_PIPE_TX)) {
            nrf24_write_multibyte_register(reg_addr, address, nrf24_hal_get_address_width());
        }
        else {
            nrf24_write_register(reg_addr, *address);
        }
    }
}
...
{
    nrf24_write_multibyte_register(W_REGISTER + TX_ADDR_REG, address, nrf24_hal_get_address_width());
}

uint8_t nrf24_hal_get_rx_payload_width(uint8_t pipe_no)
{
    return (nrf24_read_register(RX_PIPE_0_PAYLOAD_WIDTH_REG + pipe_no) & 0x3F);
}
...
void nrf24_hal_reset()
{
    ...
    nrf24_write_register(CONFIG_REG, 0x08);
    nrf24_write_register(ENABLE_AUTO_ACK_REG, 0x3F);
    nrf24_write_register(ENABLE_RX_ADDR_REG, 0x02);
    nrf24_write_register(SETUP_ADDRESS_WIDTH_REG, 0x03);
    nrf24_write_register(SETUP_RETRANSMISSION_REG, 0x03);
    nrf24_write_register(RF_CHANNEL_REG, 0x02);
    nrf24_write_register(RF_SETUP_REG, 0x0E);
    nrf24_write_register(STATUS_REG, 0x0E);
    nrf24_write_register(OBSERVE_TX_REG, 0x00);

    for (i = 0; i < 6; i++)
    {
        nrf24_write_register(RX_PIPE_0_PAYLOAD_WIDTH_REG + i, 0x00);
    }

    nrf24_write_register(FIFO_STATUS_REG, 0x11);
    nrf24_write_register(DYNAMIC_PAYLOAD_ENABLE_REG, 0x00);
    nrf24_write_register(FEATURE_REG, 0x00);
}
#endif
...
void nrf24_hal_print_details(uint8_t print_reg_value)
{
...
#if 1
    reg_value = nrf24_read_register(CONFIG_REG);
    ...
    reg_value = nrf24_read_register(ENABLE_AUTO_ACK_REG);
    ...
    reg_value = nrf24_read_register(ENABLE_RX_ADDR_REG);
    ...
    addr_width = nrf24_read_register(SETUP_ADDRESS_WIDTH_REG) + 2U;

    NRF24_DEBUG_PRINTF(printf_P(PSTR("Addr Width\t= %d Bytes\n"), addr_width);)

    reg_value = nrf24_read_register(SETUP_RETRANSMISSION_REG);
    ...
    reg_value = nrf24_read_register(RF_CHANNEL_REG);
    NRF24_DEBUG_PRINTF(printf_P(PSTR("RF Channel\t= %d\n"), reg_value);)

    reg_value = nrf24_hal_get_data_rate();

    if (reg_value == NRF24_HAL_DATA_RATE_250KBPS)
    {
        reg_value = 2;
    }
    else if (reg_value == NRF24_HAL_DATA_RATE_1MBPS)
    {
        reg_value = 0;
    }
    else
    ...
    for (i = 0; i < 6; i++)
    {
        reg_value = nrf24_read_register(RX_PIPE_0_PAYLOAD_WIDTH_REG+i);
        NRF24_DEBUG_PRINTF(printf_P(PSTR("%d,"), reg_value);)
    }

    NRF24_DEBUG_PRINTF(printf_P(PSTR("]\n"));)

    reg_value = nrf24_read_register(DYNAMIC_PAYLOAD_ENABLE_REG);
    ...
    reg_value = nrf24_read_register(FEATURE_REG);
    ...
    nrf24_read_multibyte_register(TX_ADDR_REG, &addr[0], addr_width);
    ...
    for (i = 0; i < 2; i++)
    {
        nrf24_read_multibyte_register(RX_PIPE_0_ADDR_REG + i, &addr[0], addr_width);
    ...
    for (i = 2; i < 6; i++)
    {
        reg_value = nrf24_read_register(RX_PIPE_0_ADDR_REG + i);
        addr[0] = reg_value;
    ...
    if (print_reg_value)
    {
        ...
        reg_value = nrf24_read_register(SETUP_ADDRESS_WIDTH_REG);
        ...
        reg_value = nrf24_read_register(OBSERVE_TX_REG);
        ...
        reg_value = nrf24_read_register(STATUS_REG);
        NRF24_DEBUG_PRINTF(printf_P(PSTR("STATUS\t\t0x%02x\n"), reg_value);)

        reg_value = nrf24_read_register(FIFO_STATUS_REG);
